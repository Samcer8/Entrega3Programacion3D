...
bool Collision::CheckCollision3D(Collision otherCollider) {
	bool collision2D = CheckCollision(otherCollider);
...
Bullet* Player::Shoot(Mesh mesh)
{
	Bullet* bullet = new Bullet(mesh, 1, 5, 3, pos, direction, 5);
	hasShot = true;
	return bullet;
}
...
void Enemy::RotationTowardsTarget()
{
	Vec3 direction = CalculateTargetDirection();
